...
export default {
  ...
  methods: {
    getData() {
      this.isBusy = true;
      this.$http
        .get("http://5.22.221.246/api/getWidgetPackSizesData")
        .then((response) => {
          this.isBusy = false;
    ...
    deleteData(deleteId) {
      this.$http
        .post(
          "http://5.22.221.246/api/deleteWidgetPackSize",
          { id: deleteId },
          {
            headers: {
    ...
    saveSize() {
      this.$http
        .post(
          "http://5.22.221.246/api/saveWidgetPackSize",
          { size: this.size },
          {
            headers: {
              "Content-Type": "application/json",
    ...
    buyWidgets() {
      this.$http
        .get("http://5.22.221.246/api/createWidgetOrder", {
          params: {
    ...
    editData() {
      this.$http
        .post(
          "http://5.22.221.246/api/updateWidgetPackSize",
